...
EditorView::EditorView(
    ...
    camera(sf::FloatRect(-40, 0, window.getSize().x, window.getSize().y)),
    deltaScroll(20), deltaRotation(2), deltaZoomIn(0.8f), deltaZoomOut(1.2f) {

    // this->font.loadFromFile("fonts/FreeMono.ttf");
    this->font.loadFromFile(workingDirectory + "fonts/DejaVuSans-ExtraLight.ttf");

    this->bottomLimitPx = 1;
    this->rightLimitPx = 1;

    this->setFontSize(this->fontSize);  // Important to call

    // TODO: Cambiarlo en relacion a la fontsize
    this->marginXOffset = 35;

    this->colorMargin = sf::Color(17, 18, 23);

    this->colorChar = sf::Color::White;
    this->colorSelection = sf::Color(106, 154, 232);
}

void EditorView::FontSizeScrollChangeDown() {
    if (this->fontSize <= this->MinFontSize) { std::cout << "Min" << std::endl; }
    else {
        this->fontSize--;
...
void EditorView::FontSizeScrollChangeUp() {
    if (this->fontSize >= this->MaxFontSize) { std::cout << "Max" << std::endl; }
    else {
        this->fontSize++;
        this->setFontSize(this->fontSize);
    }
